...
class FichaSemiTransparente(pg.sprite.Sprite):
    """Clase para representar una ficha semitrasparente como un sprite."""
    ...
    def cargarImagenes(cls):
        ...
        if not cls._asset_blanca:
            cls._asset_blanca = pg.image.load(c.IMG_FICHA_BLANCA_SEMITRANSPARENTE).convert_alpha()
            cls._asset_blanca = pg.transform.scale(cls._asset_blanca, (c.DIM_FICHA, c.DIM_FICHA))
        
        if not cls._asset_negra:
            cls._asset_negra = pg.image.load(c.IMG_FICHA_NEGRA_SEMITRANSPARENTE).convert_alpha()
